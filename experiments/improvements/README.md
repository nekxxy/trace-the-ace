...
| v10 | + 9 per-turn timing/latency dense features (mean/std/trend of student response latency, gap after hint vs correction, pace CV, fast/slow response ratios) on the bge-base semantic matrix | primary: 0.59185 (Δ +0.00008 vs 0.59177); session: 0.55721 (Δ +0.00007 vs 0.55714) | not submitted | — | ❌ wrong-direction noise | never built for submission |
...
**v10 (2026-07-27):** tested 9 per-turn timing/latency dense features (mean/std/trend of student response latency, longest gap, latency specifically after hint vs correction tutor turns, pace consistency, fast/slow response ratios — all pure functions of one transcript's real second-level timestamps, no label input) appended to the semantic head's dense block, same ablation method as v08. Primary delta +0.00008, session-disjoint delta +0.00007 — both point the *wrong* direction (worse), tiny magnitude consistent with noise. Unlike v08 (real-direction, too small) or H-B (real diversity, no value), this doesn't even show a believable positive direction. **Not adopted.** Full detail in `experiments/improvements/v10_timing_features.md`.
