#!/usr/bin/env python3
"""Compute graph-inspired dense features for every cached response row.

Standalone v08 exploration tool - does NOT touch feature_store.py's production
schema. Reads each session's transcript once, reuses it across every response
row that shares that session, and writes row_id -> graph feature vector to a
parquet file for downstream CV ablation (see graph_ablation_cv.py).
"""
...
from trace_ace.graph_features import GRAPH_DENSE_FEATURE_NAMES, extract_graph_features
...
def main() -> int:
    ...
    n = len(responses)
    vectors = np.zeros((n, len(GRAPH_DENSE_FEATURE_NAMES)), dtype=np.float64)
    ...
    for session_id, group in by_session:
        ...
        for row_id, objective in zip(group["row_id"], group["learning_objective"]):
            result = extract_graph_features(transcript, objective)
    ...
    frame = pd.DataFrame(vectors, columns=list(GRAPH_DENSE_FEATURE_NAMES))
